...
public class BSTree<T extends Comparable<T>> {
  ...
  ////////////////////////////////////////////////////////////////////// 
  /** Return the in-order predecessor of u in the tree, if any
  ...
  //////////////////////////////////////////////////////////////////////  
  /** Tests whether the obj equals the obj in this class
    * @param obj
    * @return true iff the obj equals this
    */ 
  ...
  private ArrayList<T> between(BSNode<T> u,T small, T large) {
    ...
    BSNode<T> p = leftmostBelow(u.left);
    while (p != ANCHOR){
      if(p.data.compareTo(small) >=0 && p.data.compareTo(large) <=0)
        ans.add(p.data);
        p=successor(p);
      }
    return ans;
  ...
  class Iterator implements ListIterator<T> {
    ...
    Iterator(boolean leftToRight) { 
      ...
      p        = (leftToRight)?leftmost:ANCHOR;
    }
    public boolean hasNext()      {
      return (successor(p) != null);
    }
    public boolean hasPrevious()  { 
      return (predecessor(p) != null);
    } 
    public T next()               {
      last = p;
      p = successor(p);
