...
//    
    ...
    System.out.println("  Testing timesAbove's execeptions");
        ...
        for(int i=0;i<10;i++) {
      int result = ts1.timesAbove(i,5,50);
      if (result!=target2[i])
      System.out.println("\tPROBLEM: ts1.timesAbove("+i+",1,4) = " 
                           + result + " but should be " + target2[i]);
    }
    ...
    System.out.println("  Testing oscillation's execeptions");
        ...
        for(int i=0;i<ts2.length();i++) {
      int result = ts2.oscillation(0,50);
      if (result!=target4[i])
        System.out.println("\tPROBLEM: ts2.oscillation(0,"+i+") = " + result 
                             + " but should be " + target4[i]);
    }
         ...
         for(int i=0;i<ts2.length();i++) {
      int result = ts2.oscillation(-50,i);
      if (result!=target4[i])
        System.out.println("\tPROBLEM: ts2.oscillation(0,"+i+") = " + result 
                             + " but should be " + target4[i]);
    }
    ...
    for(int i=0;i<ts2.length()-1;i++) {
      System.out.println("  Checking ts2.runningAvg("+(i+50)+")");
      TimeSeries result = ts2.runningAvg(i+50);
      if (!Arrays.equals(result.toArray(),tsa[i])) 
        System.out.println("\t PROBLEM: ts2.runningAvg("+(i+50)+") = "  
                             + result + "\n\t\t but should be " 
                             + Arrays.toString(tsa[i]));
    }
    
    for(int i=0;i<ts2.length()-1;i++) {
      System.out.println("  Checking ts2.runningAvg("+(-50+i)+")");
      TimeSeries result = ts2.runningAvg(-50+i);
      if (!Arrays.equals(result.toArray(),tsa[i])) 
        System.out.println("\t PROBLEM: ts2.runningAvg("+(-50+i)+") = "  
                             + result + "\n\t\t but should be " 
                             + Arrays.toString(tsa[i]));
    }
    // Add two tests that runningAvgs throws its exceptions correctly 
