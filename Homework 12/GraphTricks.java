...
//  }
  ...
  /** Tests if the vertex s is a source of the graph g
    * @param g the graph
    * @param s the start vertex
    * @return true iff vertex s is a source of g
    */
  public static boolean isSource(Graph g,int s) {
    //Check to see if there are more than 0 edges with source s 
    ...
    //Do a DFS from s in g
    dfs(g,s,info);
    //After the DFS, see if s can reach any other vertex in g
    for(int i=0; i<n; i++) {
      if(!isReachable(g,s,i)){
      }
    }
    return true;
  }
  ...
  //////////////////////////////////////////////////////////////////////
  /** Returns an array of the shortests paths from s in graph g
    * @param g the graph
    * @param s the start vertex
    * @return true iff there is a path from s in g
    */
  ...
  public static LinkedList<Integer>[] paths(Graph g, int s) {
    ...
    int x;
    int n = g.nVertices();
    //Do a BFS from s in g
    bfsList = bfs(g,s);
    //Go through BFS and add vertices into Linked List
    for(int i=0; i<n; ++i) {
      x = bfsList[i];
      ans[i].addFirst(i);
      if(i != s && x == -1){
  ...
  //////////////////////////////////////////////////////////////////////
  /** Returns a cycle in graph g, if there is one to be found
    * @param g the graph
    * @return true iff there is a cycle in graph g
    */
  public static LinkedList<Integer> findCycle(Graph g) {
    LinkedList<Integer> ans = new LinkedList<Integer>();
    VertInfo[]info = systematicDfs(g);
    //If vert 1 and vert 2 have not been touched yet,
    //add vert 2 into a linked list after vert 1 until
    //vert 1 and vert 2 are the same
    if(vert1 != -1 && vert2 != -1) {
      ans.addFirst(vert2);
      while(vert2 != vert1){
        ans.addFirst(vert1);
        vert1 = info[vert1].parent;
      }
      ans.addFirst(vert2);
    }
    return ans;
  ...
  //////////////////////////////////////////////////////////////////////
  /** Returns a list consisting of a topological sort of the graph
    * @param g the graph
