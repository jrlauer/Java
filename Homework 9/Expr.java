...
public class Expr {
  ...
  public String toInfix() {
     switch ( tag ) {
      ...
      case VAR:
        return " " + name;
      default:
        return " ( " +left.toInfix() + " " + name + " " + right.toInfix() + " ) ";
    // System.out.println("toInfix: Fix me!!");
     }
  }
  ...
  public String toPostfix() {
    switch ( tag ) {
      ...
      case VAR:
        return " " + name;
      default:
        return left.toPostfix() + right.toPostfix() + " " + name;
  ...
  public static Expr subst(Expr e1, char v, Expr e2) {
    switch (e1.tag) {
      case NUM:
        return e1;
      case VAR:
        if(e1.name == v) return e2;
        else return e1;
      default:
        return new Expr(subst(e1.left,v,e2), e1.name, subst(e1.right,v, e2));
  ...
  public Expr simplify() {
    switch ( tag ) {
      ...
      default:
        if (left.tag.toString().equals("NUM") && right.tag.toString().equals("NUM"))
          return new Expr(eval(new HashMap<Character,Double>()));
        
        else if(left.simplify().tag.toString().equals("NUM") && right.simplify().tag.toString().equals("NUM"))
          return new Expr(eval(new HashMap<Character,Double>()));
        
        else
          return new Expr(left.simplify(), name, right.simplify());
